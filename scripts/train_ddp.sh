#!/usr/bin/env bash
# UNITE training launcher — edit the vars below to change settings.
set -euo pipefail
...
if [ "$NPROC" -gt 1 ] && [ -n "$GPUS" ]; then
    echo "Launching DDP training on GPUs: $GPUS (nproc=$NPROC)"
    EXTRA_ARGS=()
    ...
    echo "Launching single-GPU training on: $DEVICE"
    if [ -n "$LOG_DIR" ]; then
        python train.py \
            --config "$CONFIG" \
            --device "$DEVICE" \
